...
int main(int argc, char *argv[])
{
    ...
    while (1) 
    {
        ...
        if (strcmp(cmd, "ls")==0) list_file();
        if (strcmp(cmd, "cd")==0) change_dir();
        if (strcmp(cmd, "pwd")==0) pwd(running->cwd);
        if (strcmp(cmd, "quit")==0) myQuit();
        if (strcmp(cmd, "mkdir")==0) mk_dir();
        if (strcmp(cmd, "creat")==0) create_fileProxy();
        if (strcmp(cmd, "link")==0) linkProxy();
